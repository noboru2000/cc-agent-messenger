...
# claude-messenger — live-session operating procedure (C0 monitor mode)
...
1. The daemon is running: `claude-messenger daemon`.
2. Read `send_api_endpoint` and `inbound_event_path` from
   `.claude-messenger/config.toml`. Export the socket so the CLI reaches the
   daemon: `export SEND_API_ENDPOINT=<send_api_endpoint>`.
3. Confirm the return path before watching: `claude-messenger ping` → expect
...
| `trigger` | Do |
|---|---|
| `health_check` | `claude-messenger ping`; reply briefly (e.g. "稼働中") |
...
| `system_doctor` | run `claude-messenger doctor` and reply with a redacted summary |
...
## Step 3 — reply
    ...
    claude-messenger send --thread <thread_ts> --correlation-id <correlation_id> --text "<reply>"
...
- Stay within the closed handler set; destructive/irreversible/outbound actions
  ...
  first.
- If `claude-messenger send` returns `halted`, the kill switch is engaged — stop
