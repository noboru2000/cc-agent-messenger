...
| `watch` | a `!watch` command — the daemon already applied it. Acknowledge from the text (e.g. "監視 gpu を 5 分ごとに開始"). For `!watch list`, run `cc-agent-messenger monitors` and relay the list. |
| `monitor_tick` (timer tick) | a scheduled monitor fired. **Gather** the content in `args` — run the `args.probe` **verbatim** if set (read-only only) and interpret `args.items` (e.g. SSH `nvidia-smi`, `tail` a log) — then reply with a **concise report + interpretation** (trend). **Evaluate `args.alert`**: if a rule trips (e.g. `temperature.gpu > 85`, loss NaN/diverging, process down), make it a **prominent ⚠️ alert** (you are already @-mentioned). Never run a non-read-only remote action without explicit NN5 approval. |
