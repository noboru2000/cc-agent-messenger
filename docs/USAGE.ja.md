...
> daemon はSlackのデスクトップ版・モバイル版が生成する両方のbotメンション表現を受理します。
> Slack内部のイベント種別が異なっても、モバイルからのトップレベル `@bot !コマンド` は
> デスクトップ版と同じように動作します。

### トップレベルとスレッドでのmention要否

| 投稿場所 | botへのmention | 例 |
|---|---|---|
| チャネルのトップレベル | **必須** | `@<bot名> !status` |
| botメッセージ配下のスレッド返信 | **不要**（付けても処理可能） | `!status` または `状況を教えて` |

mentionのないトップレベル投稿は取り込みません。これにより、通常のチャネル会話を誤って
agentへの指示として扱うことを防ぎます。スレッド内では、そのスレッド自体が会話の宛先に
なるため、iPhoneを含むSlackクライアントからmentionなしで続けられます。
