...
2. **OAuth & Permissions → Bot Token Scopes** で次を追加します。
   ...
   (👀→✅のレシートには`reactions:write`、repo／agentごとの送信者表示名には
   `chat:write.customize`が必要です。)
...
4. **Event Subscriptions → Enable。** 「Subscribe to bot events」に
   ...
   必須です。これがないとイベントが届きません。)`message.groups` はスレッド返信に加え、Slack
   モバイル版のトップレベルbotメンションにも必要です。モバイル版のメンションは
   `app_mention` ではなく通常の `message` として届く場合があります。
...
続いて、**daemonを起動する前にSlackアプリと設定を確認します**。まず読み取り専用の診断を
実行します。daemonは不要です。

    cc-agent-messenger doctor --slack

`--slack` は認証、**付与済みスコープ**、チャネルへの参加、Socket Modeを点検します。
`chat:write`、`chat:write.customize`、`app_mentions:read`、`groups:history`は
すべてPASSが必要です。
`groups:history` が、モバイル版のトップレベルメンションを含む `message.groups` の受信を許可します。
任意のリアクション／スラッシュ用スコープが不足している場合は推奨事項として表示されます。

次に、tokenからは取得できない設定をSlack管理画面で手動確認します。

1. <https://api.slack.com/apps> → 対象app → **Event Subscriptions** を開く。
2. **Enable Events** がOnであることを確認する。
3. **Subscribe to bot events** に `app_mention`、`message.groups`、`reaction_added` があることを
   確認し、変更した場合は **Save Changes** を押す。
4. **OAuth & Permissions** で `groups:history` を追加した場合はappをworkspaceへ再インストールし、
   現在の `xoxb-…` tokenを `config.toml` に設定する。

Slackはインストール済みbot/app tokenを使ってEvent Subscriptionsを読み出すAPIを提供していないため、
`doctor` は `message.groups` の購読状態を自動確認できません。任意の送信・レシート実走診断
(使い捨てメッセージを投稿します)は次で実行します。

    cc-agent-messenger doctor --slack --live

これは投稿と 👀→✅ を確認しますが、`message.groups` の受信確認にはなりません。daemon起動後、
Slackモバイル版から新しいトップレベル `@<bot名> !help` を送り、👀と1件の返信が来ることを
確認します。デスクトップ版でも繰り返し、返信が二重にならないことを確認します。
(ローカルのsocket／ping確認は§6で行います。)
...
## 7. 常駐Claude Codeセッションを起動する(C0)

ここが、Slack コマンドに**返信する**役割を担う部分です。

C0は、起動済みの対話型Claude Codeセッションが受信ファイルを監視し、作業コンテキストを保持する
方式です。VS CodeのClaude Codeウィンドウでも、対話型`claude` CLIでもC0として利用できます。
C1は別方式で、Slackの1ターンごとにdaemonがheadless CLIを起動します。v0.6へ更新しても、§9の
`integration = "c1"` agentを明示的に追加しない限りC1には切り替わりません。
...
### VS CodeのClaude CodeウィンドウでC0を使う
...
### 対話型Claude Code CLIでC0を使う

terminal上でagentを常駐させる場合に使います。オーナー管理下のリモート開発ホストでも同じです。
daemonと対話型Claudeセッションは別々の常駐プロセスです。

1. プロジェクトディレクトリで、先にdaemonを確認します。

       cd <your-project>
       cc-agent-messenger ping

2. 同じプロジェクトで対話型Claude Codeセッションを起動し、そのまま常駐させます。

       cd <your-project>
       claude

3. Claudeの入力欄で `/cc-agent-messenger` を実行します。slash commandの一覧に出ない場合は、
   `cc-agent-messengerスキルを使い、C0 monitor modeでSlackメッセージを待ち受けて` と依頼します。
   セッションはプロジェクトのskillを読み、設定された受信ファイルに `tail -n 0 -F` Monitorを接続し、
   `cc-agent-messenger send` で返信します。
4. 対話型ClaudeプロセスとMonitorを常駐させます。CLI終了、セッション終了、Monitor停止のいずれでも
   C0の返信は止まります。daemonはイベントを受信し、キューへ残している場合があります。
5. 再接続後はMonitorを再開する前に `cc-agent-messenger pending` を実行し、未処理分へ返信してから
   処理済みイベントをackします。

アップグレード時は別terminalで `cc-agent-messenger init` とdaemon再起動を行います。常駐中の
Claude CLIには、更新されたskillを読み直してC0 Monitorを再接続するよう依頼します。C1へ切り替える
必要はありません。
...
**[コマンドリファレンス → docs/USAGE.ja.md](USAGE.ja.md)** にまとめてあります。
...
- 作業を担当する常駐Claude Code CLI／ウィンドウがあり、そのライブコンテキストを維持したい場合は
  **C0**を選びます。既定の単一チャネル構成はC0で、`[[agent]]`追加は不要です。
- Slackメッセージが来たときだけdaemonにagentを起動させる場合は**C1**を選びます。C1には明示的な
  `[[agent]]`設定が必要で、C0 Monitorは使いません。
- 同じSlackチャネルをC0とC1の両方へ割り当てないでください。設定済みagent routeがそのチャネルを
 受け持つため、メッセージは既定C0の受信ファイルへも同時には入りません。

### repo／agentごとの送信者名を設定する

関連する4種類の名前は、意図的に分離されています。

| 名前 | 例 | 制御するもの |
| --- | --- | --- |
| Slack App／メンション名 | `@MacMessenger`、`@Messenger` | Appのinvite／メンション時に使う名前。Slack側で設定 |
| `default_agent` | `MacMessenger`、`Messenger` | 既定・未ルーティングC0チャネルの投稿表示名 |
| `[[agent]].name` | `claude-headless` | 安定した内部routing名／C1 resume session key |
| `[[agent]].display_name` | `ULBC Claude` | そのC0／C1チャネルから投稿するときのSlack表示名 |

`display_name`を変更してもSlack App自体は改名されず、`@`の後に入力するメンション名も変わりません。
逆に、Slack側でAppの表示／メンション名を変更しても、このrepoのconfigは書き換わりません。

既定C0チャネルは`default_agent`をSlackの送信者表示名として使います。

    default_agent = "ULBC Mac"

ルーティングされるC0／C1 agentでは、安定した内部名と表示専用名を分けられます。

    [[agent]]
    name = "claude-headless"
    display_name = "ULBC Claude"

`display_name`省略時は`name`を表示します。`display_name`の変更ではC1 threadはリセットされません。
`name`はsession keyでもあるため、安易に変更しないでください。表示名は信頼済みlocal configだけから
決まり、Slack本文やsend CLIからは上書きできません。変更後はdaemonを再起動します。

MacのC0 Appと、rtxuのC0／C1 Appを併用する例:

```toml
# Mac repo: 既定C0は「MacMessenger」として投稿
default_agent = "MacMessenger"
```

```toml
# rtxu repo: 既定C0は「Messenger」として投稿
...
1. **OAuth & Permissions → Bot Token Scopes**へ`chat:write.customize`を追加する。
2. Slack Appをworkspaceへ再インストールする。
3. Slackが新しい`xoxb-…` tokenを表示した場合、localの`config.toml`を更新する。
4. `cc-agent-messenger doctor --slack`を実行し、必須scopeがすべてPASSすることを確認する。
5. daemonを再起動し、C0／C1から1件ずつ返信して投稿表示名を確認する。既存Slack投稿の表示名は
   過去のままです。

`cc-agent-messenger init`は安全なupgradeを優先し、既存projectでは同梱skillを更新しても
**`config.toml`を上書きしません**。`default_agent`／`display_name`は手動で追加・編集します。
新規`init`は最新exampleをコピーし、既存configはtoken、path、route、custom表示名を保持します。
...
4. **ライブセッションを再接続する — ウィンドウのリロードは不要。** 「Developer: Reload Window」は
   ...
   *§7 → コピペプロンプト* の**②更新を反映のプロンプト**を貼り付けます。セッションが更新後のスキルを
...
- **設定した送信者名が使われずSlack App全体の名前で表示される** → install済みtokenに
  `chat:write.customize`がない、scope追加後にAppを再インストールしていない、またはdaemonが古い
  config／tokenを保持しています。`cc-agent-messenger doctor --slack`を実行し、Slackが新しい
  `xoxb-…`を発行していれば更新してdaemonを再起動します。v0.7ではscope欠落はFAILです。
- **`display_name`変更後にC1会話が新規になった** → `name`も変更していないか確認してください。
  保存済みresume sessionのkeyは`name`だけなので、これは固定します。
- **メンションは`@MacMessenger`だが返信表示は`ULBC Claude`** → 正常です。`@`名はSlack App、
  `display_name`は送信メッセージの表示名だけを制御します。
